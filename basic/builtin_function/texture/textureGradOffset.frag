...
void main()
{
  ...
  temp1 += textureGradOffset(tex13, coord.xyz, dx.xy, dy.xy, offset.xy);
  temp1 += textureGradOffset(tex4, coord.xyz, dx.x, dy.x, offset.x);
  temp1 += textureGradOffset(tex5, coord.xyz, dx.xy, dy.xy, offset.xy);
  temp1 += textureGradOffset(tex9, coord.xyz, dx.x, dy.x, offset.x);
  temp1 += textureGradOffset(tex10, coord.xyzw, dx.xy, dy.xy, offset.xy);
  
  temp += textureGradOffset(tex0, coord.x, dx.x, dy.x, offset.x);
  temp += textureGradOffset(tex1, coord.xy, dx.xy, dy.xy, offset.xy);
  temp += textureGradOffset(tex2, coord.xyz, dx.xyz, dy.xyz, offset.xyz);
  temp += textureGradOffset(tex12, coord.xy, dx.xy, dy.xy, offset.xy);
  temp += textureGradOffset(tex6, coord.xy, dx.x, dy.x, offset.x);
  temp += textureGradOffset(tex7, coord.xyz, dx.xy, dy.xy, offset.xy);
  temp += textureGradOffset(tex14[index], coord.xy, dx.xy, dy.xy, offset.xy);
