...
void main()
{
  ...
  temp1 += textureProjGrad(tex13, coord.xyzw, dx.xy, dy.xy);
  temp1 += textureProjGrad(tex4, coord.xyzw, dx.x, dy.x);
  temp1 += textureProjGrad(tex5, coord.xyzw, dx.xy, dy.xy);
  
  
  temp += textureProjGrad(tex0, coord.xy, dx.x, dy.x);
  temp += textureProjGrad(tex0, coord.xyzw, dx.x, dy.x);
  temp += textureProjGrad(tex1, coord.xyw, dx.xy, dy.xy);
  temp += textureProjGrad(tex1, coord.xyzw, dx.xy, dy.xy);
  temp += textureProjGrad(tex2, coord.xyzw, dx.xyz, dy.xyz);
  temp += textureProjGrad(tex12, coord.xyw, dx.xy, dy.xy);
  temp += textureProjGrad(tex12, coord.xyzw, dx.xy, dy.xy);
  temp += textureProjGrad(tex14[index], coord.xyw, dx.xy, dy.xy);
