...
void main()
{
  ...
  temp1 += textureGrad(tex13, coord.xyz, dx.xy, dy.xy);
  temp1 += textureGrad(tex4, coord.xyz, dx.x, dy.x);
  temp1 += textureGrad(tex5, coord.xyz, dx.xy, dy.xy);
  temp1 += textureGrad(tex9, coord.xyz, dx.x, dy.x);
  temp1 += textureGrad(tex10, coord.xyzw, dx.xy, dy.xy);
  
  temp += textureGrad(tex0, coord.x, dx.x, dy.x);
  temp += textureGrad(tex1, coord.xy, dx.xy, dy.xy);
  temp += textureGrad(tex2, coord.xyz, dx.xyz, dy.xyz);
  temp += textureGrad(tex3, coord.xyz, dx.xyz, dy.xyz);
  temp += textureGrad(tex12, coord.xy, dx.xy, dy.xy);
  temp += textureGrad(tex6, coord.xy, dx.x, dy.x);
  temp += textureGrad(tex7, coord.xyz, dx.xy, dy.xy);
  temp += textureGrad(tex8, coord.xyzw, dx.xyw, dy.xyw);
  temp += textureGrad(tex14[index], coord.xy, dx.xy, dy.xy);
