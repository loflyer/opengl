...
layout(binding=8) uniform sampler2DArray tex8[4];
...
void main()
{
  ...
  temp += texelFetchOffset(tex0, icoord.x, icoord1.z, offset.x);
  temp += texelFetchOffset(tex1, icoord.xy, icoord1.z, offset.xy);
  temp += texelFetchOffset(tex2, icoord.xyz, icoord1.z, offset.xyz);
  temp += texelFetchOffset(tex12, icoord.xy, offset.xy);
  temp += texelFetchOffset(tex6, icoord.xy, icoord1.z, offset.x);
  temp += texelFetchOffset(tex7, icoord.xyz, icoord1.z, offset.xy);
  temp += texelFetchOffset(tex8[index], icoord.xyz, icoord1.z, offset.xy);
